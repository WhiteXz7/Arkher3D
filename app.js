...
const Arkher = {
  ...
  async ask(messages, opts = {}) {
    ...
    if (opts.visao && !forced) {
      const VE = /gpt-?4|gpt-?5|4o|fable|claude|sonnet|opus|haiku|gemini|pixtral|llava|qwen.*vl|intern-?vl|molmo|vision|multimodal|grok.*vision|llama.*(3\.2|4).*(11|90|vision|scout|maverick)/i;
      const f2 = fila.filter(x => VE.test(x.id));
