...
(function () {
  ...
  async function tela() {
    try {
      const r = await Pilot.vm('/screen?scale=0.5&q=55');
      if (r.ok) { pinta(r.img); estado('conectado', true); return true; }
      estado('erro: ' + (r.err || '').slice(0, 40), false);
    } catch (e) { estado('agente offline', false); }
    return false;
  }
  ...
  img.onclick = async ev => {
    ...
    const x = Math.round((ev.clientX - b.left) / b.width * img.naturalWidth / escala);
    const y = Math.round((ev.clientY - b.top) / b.height * img.naturalHeight / escala);
    await Pilot.vm('/input', { acts: [{ do: 'click', x, y }] });
    setTimeout(tela, 500);
  };
  ...
  async function go() {
    ...
    let trava = true;
    try { if (window.Sync && Sync.pegarTrava) trava = await Sync.pegarTrava(600); } catch (e) {}
    if (!trava) { diz('err', 'VM ocupada por outra pessoa — aguarde'); fim(); return; }
    try { if (window.Sync && Sync.log) Sync.log('piloto', obj.slice(0, 120)); } catch (e) {}
  ...
  $('#pl-stop').onclick = () => { rodando = false; diz('', 'parando após o passo atual…'); };
  $('#pl-go').onclick = go;
